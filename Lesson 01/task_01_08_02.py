...
def performNamespace(namespaceName):
    namespace = [namespaceName]
    print("namespace", namespace)
    global i # Счетчик запросов
    while i <= n:
        request = input().split(" ")
        i += 1
        if request[0] == "create":
            if request[2] in namespace:
                namespace.append(request[1])
                performNamespace(request[1])
            else:
                print("Variant create 2")
                return -1
        elif request[0] == "add":
            if request[1] in namespace:
...
# for i in range(n):
    request = input().split(" ")
    i += 1
    if request[0] == "create": # Проверить на глобальность!
        namespace.append(request[1])
        performNamespace(request[1])
    elif request[0] == "add": # Проверить на глобальность!
        namespace.append(request[2])
    print("namespace", namespace)
