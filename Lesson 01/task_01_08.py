...
def appendNamespace(namespace, parent, currentNamespace, perviousNamespace):
    if currentNamespace == parent:
        dict[parent] = currentNamespace.get(parent, namespace)



namespaceList = []
namespaceDict = {"global": {}}
requestDict = {}
currentDictLink = namespaceDict
pervDictLink = namespaceDict
n = int(input())
for _ in range(n):
    request = input().strip().split()
    if request[0] == "create":
        appendNamespace(request[1], request[2], currentDictLink, pervDictLink)

    #     requestDict[request[1]]
